...
class Birthdays:
    ...
    YEAR_INVALID = ":x: The year you entered is invalid. You can't be that old."
    ...
    async def bday_set(self, ctx, date, year: int=None):
        ...
        if year is not None:
            ...
            if age < 0 or age > 120:
                await self.bot.send_message(channel, self.YEAR_INVALID)
                return
        self.remove_user_bday(author.id)
        ...
        for k, g in itertools.groupby(sorted(datetime.datetime.fromordinal(int(o)) for o in bdays.keys()),
            ...
            for date in g:
                if len(bdays.get(str(date.toordinal()))) > 0:
                    today_d = bdays.get(str(date.toordinal()), {}).items()
                    people_today = []
                    for u_id, year in today_d:
                        if message.server.get_member(u_id) is not None:
                            people_today.append("<@!{}>{}".format(u_id,
                                "" if year is None\
                                    else " ({})".format(this_year-int(year))))
                    today_s = date.strftime("%d").lstrip("0") + ": " + \
                            ", ".join(people_today)
                    #days.append(today_s)
                    if len(people_today) > 0:
                        days.append(today_s)
            month_string = "\n".join(days)
            if len(month_string) > 0:
                embed.add_field(name=datetime.datetime(year=1, month=k, day=1).strftime("%B"), value=month_string)
